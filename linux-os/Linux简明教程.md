...
- [Linux Concise Tutorial](#linux-concise-tutorial)
  - [Linux 简介](#linux-简介)
    ...
    - [常用命令(必须熟记)](#常用命令必须熟记)
      - [基本辅助指令](#基本辅助指令)
      - [关闭与重启系统](#关闭与重启系统)
      - [**`du` 和 `df` 的区别**](#du-和-df-的区别)
  - [Linux 常用操作](#linux-常用操作)
    - [目录操作](#目录操作)
    - [文件操作命令](#文件操作命令)
    - [打包和解压命令](#打包和解压命令)
    - [网络命令](#网络命令)
    - [权限指令 `chmod`](#权限指令-chmod)
  - [Vim 命令基础](#vim-命令基础)
    - [**操作步骤示例**](#操作步骤示例)
    - [**补充**](#补充)
...
| 命令 | 全称 / 含义         | 查看范围                  | 实现方式              | 输出结果说明                                   |
| ---- | ------------------ | ------------------------ | --------------------------- | ------------------------------------- |
| `df` | “**disk free**”／磁盘可用    | 每个 **已挂载的文件系统**（或分区）整体情况 | 通过文件系统元数据（如 superblock、`statfs` 系统调用）查询剩余／已用块数。 ([Ask Ubuntu][1]) | 显示文件系统总大小、已用、可用、使用百分比、挂载点。适合快速查看哪个分区快满了。 |
| `du` | “**disk usage**”／磁盘使用量 | 某个目录或文件（及其子目录）实际所占用空间   | 遍历目录及其下所有文件，累加每个文件的占用空间。 ([OSTechNix][2])                          | 显示某目录／文件及其子树占用了多少空间。适合找到“哪个目录占磁盘最多”。     |

[1]: https://askubuntu.com/questions/280342/why-do-df-and-du-commands-show-different-disk-usage "Why do \"df\" and \"du\" commands show different disk usage?"
[2]: https://ostechnix.com/understanding-df-and-du-commands-in-linux/ "Understanding df And du Commands In Linux - OSTechNix"


```bash
# 说明：显示文件系统的磁盘使用情况
df -h            #  一种易看的显示(h表示human-readable, 人类可读单位（如 K, M, G）)
df -l            #  只显示本地磁盘

du -sh /path/to/dir   
# -s：只汇总，不列出子目录；-h 可读单位
du -h --max-depth=1 /path/to/dir
# 显示该目录下第一层子目录分别占用多少空间
du -ah /path/to/dir | sort -hr | head -n 10
# -a：包括所有文件；列出占空间最大的前10项
```

**同一个分区 df 和 du 的数值区别**

- `df` 报的是文件系统级别的“已使用块数 + 保留块 +文件系统元数据（inode表、日志、空闲保留空间）”情况;
- `du` 报的是从目录树中能“看到”的文件大小总和，不包括某些情况（如已删除但被进程打开的文件／挂载遮蔽的目录）所占的空间

**使用场景？**

- 若你想了解 磁盘或分区整体剩余空间/使用情况，用 `df`;
- 若你想查找 哪个目录或文件占用了大量空间，用 `du`;
- 对于磁盘“满了”但看不出在哪个目录的问题，可以先用 `df` 看哪个分区满了，再用`du` 在那个分区内查找具体目录[redhat.com](https://www.redhat.com/en/blog/du-vs-df).
...
1. [Linux学习教程，Linux入门教程（超详细）-C语言中文网](https://c.biancheng.net/linux_tutorial/);
1. [Linux教程-runoob.com](https://www.runoob.com/linux/linux-tutorial.html);

1. [Linux 常用操作命令大全（最后更新时间：2024年1月）-CSDN](https://blog.csdn.net/m0_46422300/article/details/104645072)  
1. [（让程序在linux下持续运行）五种方法维持远程服务器的session在断开连接后不被中断（详细解释）-csdn](https://blog.csdn.net/qq_34769162/article/details/107948168)
   
可以使用以下网页查询相关Linux 命令:


1. [Linux命令大全(手册)](https://www.linuxcool.com/)
2. [Linux 命令大全速查表-一份包含 491 个 Linux 命令的极简指南](https://linux-commands.labex.io/zh/)
